...
# ----- generate_url() -----
# Build the Wikipedia URL(s) for a given election year.
#   * 2019 has two separate elections (April & November), so it
#     returns 2 links.
...
generate_url <- function(years) {
  ...
  if (years == 2019) {
    ...
    urls <- paste0("https://en.wikipedia.org/wiki/Opinion_polling_for_the_", years, "_Spanish_general_election")
  }
...
# We need to find all tables after the h4
# <h4 id="Voting_intention_estimates"> but before any other h4 or
# higher
get_tables_after_voting_estimates <- function(url) {

  # Read page and locate the <div> that wraps the
  # <h4 id="Voting_intention_estimates">
  siblings <- url |>
    read_html() |>
    # starting node: has to be the parents => the div that contains
    # <h4 id='Voting_intention_estimates'>
    xml_find_first(".//div[h4[@id='Voting_intention_estimates']]")  |>
    # get all * following sibling tables after the one we just
    # mentioned (div of h4) (https://www.roborabbit.com/blog/mastering-xpath-using-the-following-sibling-and-preceding-sibling-axes/)
...
# ----- extract_party_names() -----
# From every xml <table>, grab the tiny coloured‑box headers that
# contain the party names. Those <th> cells have fixed widths
...
# ----- get_links_after_voting_estimates() -----
# Some "Voting intention" tables are not on the main page but on
# subpages linked immediately after an h5 header via a *hatnote*.
# This function collects those links together with the year they
# belong to.
...
# ----- get_tables_from_links() -----
# Drill into the subpages collected above, harvesting
...
# ----- extract_polling_data() -----
# Main orchestrator for one election year URL.
